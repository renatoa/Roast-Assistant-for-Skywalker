REM update the Arduino paths AND COM port, according to your setup
REM this batch file and the .bin file must be placed in same folder
C:\Users\Renato\AppData\Local\Arduino15\packages\esp32\tools\esptool_py\4.5.1/esptool.exe --chip esp32s3 --port COM39 --baud 921600 --before default_reset --after hard_reset write_flash -z --flash_mode dio --flash_freq 80m --flash_size 4MB 0x0 ^
